...
function readAppSettings(values) {
  var jsonPath = values.root.append('package.json')
  ...
  function parseJSON(text) {
    ...
    try {
      ...
      err = new Error("JSON SyntaxError: "+ err.message +" in "+ jsonPath.toString());
      return LIB.Promise.reject(err);
    }
    return app;
  }

  var promise = jsonPath.read()
    .then(parseJSON)
    .then(function (meta) {
      meta = meta || Object.create(null);
      meta.name = meta.name || 'enginemill-default';
...
function loadSettingsPath(values) {
  var promise
    , appname                 = values.settings.application.name
...
// - values.settings
//
// Sets values.settings.facade() method
//
// Returns values
function setSettings(values) {

  Object.defineProperty(values.settings, 'facade', {
    enumerable: true
  , value: function () {
      var self = this
