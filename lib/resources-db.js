...
class Resource {
	...
	static create(spec = {}) {
		...
		if (!Resource.TYPES_LIST.includes(spec.type)) {
			throw new Error(`Resource.create() invalid spec.type "${spec.type}"`);
		}
...
Resource.TYPES_LIST = Object.entries(Resource.TYPES).map((items) => items[1]);
...
class ResourcesDB {
	...
	createResource(metaFile, dataFile) {
		...
		if (metaFile) {
			...
			const extname = dataFile.getExtensionName().replace(/^./, '');
			const type = Resource.TYPES[extname];

			if (!type) {
				return Promise.reject(new Error(`Invalid resource data file extension "${extname}"`));
			}
	...
	static createMetaFile({ directory, type }) {
		if (!Resource.TYPES_LIST.includes(type)) {
